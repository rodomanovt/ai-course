...
def report(
    ...
    title: str = typer.Option("EDA-отчёт", help="Заголовок отчета report.md"),
    min_missing_share: float = typer.Option(0.5, help="Доля пропусков, выше которой колонка считается проблемной")
) -> None:
    """
    ...
    if not problematic_cols_df.empty:
        problematic_cols_df.to_csv(out_root / "problematic_columns.csv", index=True)
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        f.write(f"- user_id не уникален: **{quality_flags['has_suspicious_id_duplicates']}**\n\n")
        ...
        f.write("См. файл `problematic_columns.csv`.\n\n")
    ...
    typer.echo("- Табличные файлы: summary.csv, missing.csv, correlation.csv, top_categories/*.csv, problematic_columns.csv")
