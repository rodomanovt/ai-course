...
def problematic_columns(
    ...
    for column in summary.columns:
        if column.missing_share >= min_missing_share:
            problematic_columns.append(column.name)
            missing_shares.append(column.missing_share)
    result = pd.DataFrame(
        {
            "column_name": problematic_columns,
            "missing_share": missing_shares
        }
    )
    return result
...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    has_sus_id_dupes = False
    for el in summary.columns:
        if el.name == "user_id":
            if el.unique != el.non_null:
                has_sus_id_dupes = True
    flags["has_suspicious_id_duplicates"] = has_sus_id_dupes

    # флаг, показывающий, есть ли колонки, где все значения одинаковые
    has_constant_columns = any(column.unique == 1 for column in summary.columns)
